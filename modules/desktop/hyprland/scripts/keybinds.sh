...
# get_nix_value() {
#     grep "$1" "$HOME/ZyNixOS/flake.nix" | sed -E 's/.*"([^"]+)".*/\1/'
# }
get_nix_value() {
    awk '
...
yad \
  ...
  "SUPER SHIFT ALT 1-0" "Move to workspace 10-20" "movetoworkspace 10-20" \
  "SUPER SHIFT ALT 1-0" "Silently move to workspace 10-20" "movetoworkspacesilent 10-20" \
